...
/**
 * Drives the extension's own toolbar UI. The toolbar lives in a closed shadow
 * root, so every lookup goes through CDP rather than page selectors.
 */
...
/**
 * Closes any host "jump menu" / "skip to" overlay that can steal the pointer.
 * A no-op on pages that have none.
 */
